...
public class ServletPayment extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			request.setAttribute("exchangeRate", new BankManagerServiceLocator().getBankManager().getExchangeRate(currency));
		...
		switch(action){ 
            ...
            case "payment": 
        		...
        		try {
        			request.setAttribute("paiementSuccess", new BankManagerServiceLocator().getBankManager().isValidPayment(cardnumber, cvv, expiracydate, name, currency, totalPrice));
        		}catch (RemoteException | ServiceException e) {
        			e.printStackTrace();
        		}
